...
def smart_sleep_until_open():
    ...
    if seconds_until_open > 3600:
        ...
        logger.info(f"💤 距離下次開盤還很久，切換至長睡眠模式...")
        logger.info(f"預計睡眠 {sleep_seconds/3600:.1f} 小時")
        logger.info(f"下次喚醒時間：台灣時間 {wake_up_time.strftime('%m/%d %H:%M')} (開盤前 30 分鐘)")
        
        time.sleep(sleep_seconds)
    else:
        # 如果距離開盤不到 1 小時，就進入「待機模式」，每 60 秒檢查一次
        logger.info(f"⏳ 快開盤了，待機中...")
        time.sleep(60)
    ...
    for symbol in STOCKS:
        try:
            ...
            if current_tier_level > last_recorded_level:
                logger.warning(f"🚨 {symbol} 跌幅 {drop_magnitude*100:.2f}%，觸發 {trigger_pct*100:.0f}% 門檻")
                ...
                logger.info(f"{symbol:<5}: ${current_price:>7.2f} | 跌幅: {change_pct*100:>6.2f}% (目前等級: {last_recorded_level})")

        except Exception as e:
            logger.error(f"讀取 {symbol} 失敗: {e}")
...
if __name__ == "__main__":
    logger.info("=== 智慧型省電美股監控系統 ===")
    logger.info(f"監控名單: {STOCKS}")
    logger.info(f"機制: 盤中每 10 分鐘檢查，收盤後自動長睡眠至開盤前")
    logger.info("----------------------------------------")
